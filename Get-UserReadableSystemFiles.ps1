...
$ErrorActionPreference='SilentlyContinue';gci "$ENV:windir\system32" -recurse |? {$_.PSISContainer -ne 'true'} | %{get-acl $_.FullName} | ? {$_.AccessToString -like "*USERS*ALLOW*READ*" -and $_.Owner -like "*SYSTEM*"}  | ft path -wrap; $ErrorActionPreference='Continue'
}
